...
const isLoggedIn = Boolean(
  ...
  return (
    ...
    >
      <div className="container mx-auto">
        <div className="relative rounded-2xl border border-[#D4AF37]/20 bg-[#080704]/95 shadow-2xl px-4 md:px-6 py-1.5 flex items-center justify-between gap-4">
          {/* Logo */}
          <div className="flex items-center gap-4">
            ...
            >
              <div className="relative">
                ...
                />
                <div className="absolute inset-0 bg-[#D4AF37]/25 blur-xl opacity-0 group-hover:opacity-100 transition-opacity" />
              </div>
          ...
          {/* Desktop Navigation */}
          <div className="hidden lg:flex items-center gap-1.5 p-1 bg-black/60 rounded-xl border border-[#D4AF37]/15">
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  {isActive && (
                    <motion.div
                      layoutId="nav-pill"
                      className="absolute inset-0 bg-[#D4AF37]/10 rounded-lg border border-[#D4AF37]/20 shadow-inner"
  ...
  <Button
    ...
    size="sm"
    variant="outline"
    className="hidden h-8 items-center gap-2 rounded-lg border-[#D4AF37]/20 bg-[#D4AF37]/[0.04] px-3.5 text-xs font-semibold text-white hover:bg-white/10 hover:text-white sm:flex"
  >
    <Link to="/history">
      ...
      <span>My Music</span>
            ...
            {!isInstalled ? (
              <Button
                ...
                className="hidden sm:flex items-center gap-2 rounded-lg px-3.5 bg-gradient-to-r from-[#A87912] via-[#D4AF37] to-[#F1D36A] text-black hover:brightness-110 font-semibold transition-all h-8 shadow-[0_0_18px_rgba(212,175,55,0.20)] text-xs"
              >
                <Download className={`w-3.5 h-3.5 ${isInstallable ? "animate-bounce" : ""}`} />
                <span>Install Studio</span>
              </Button>
            ) : (
              <div className="hidden sm:flex items-center gap-2 px-3 py-1 rounded-lg bg-[#D4AF37]/5 border border-[#D4AF37]/15 text-[#D4AF37]/60 text-[10px] uppercase font-bold tracking-wider">
            ...
            <Sheet open={mobileMenuOpen} onOpenChange={setMobileMenuOpen}>
              <SheetTrigger asChild>
                <Button
                  variant="ghost"
                  size="icon"
                  className="lg:hidden text-[#F4D06F] bg-[#D4AF37]/10 border border-[#D4AF37]/20 rounded-lg w-8 h-8"
                >
              ...
              <SheetContent
                side="right"
                className="w-[300px] bg-[#080704] border-[#D4AF37]/20"
              >
                <SheetHeader className="pb-6 border-b border-[#D4AF37]/15">
    ...
    return (
      <Link
        ...
        className={`flex items-center gap-3 rounded-lg px-4 py-3 text-sm font-medium transition-all ${
          isActive
            ? "border border-[#D4AF37]/20 bg-[#D4AF37]/10 text-[#F4D06F]"
    ...
    <Link
      ...
      className={
        location.pathname === "/history"
          ? "flex items-center gap-3 rounded-lg border border-[#D4AF37]/20 bg-[#D4AF37]/10 px-4 py-3 text-sm font-medium text-[#F4D06F]"
